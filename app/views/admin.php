...
// Get stats safely
try {
  $showCount = $pdo->query("SELECT COUNT(*) FROM shows")->fetchColumn();
  $eventCount = $pdo->query("SELECT COUNT(*) FROM events")->fetchColumn();
  $userCount = $pdo->query("SELECT COUNT(*) FROM users")->fetchColumn();
  $trackedCount = $pdo->query("SELECT COUNT(*) FROM user_tracked_shows")->fetchColumn();
  $intCount = $pdo->query("SELECT COUNT(*) FROM user_integrations WHERE enabled = 1")->fetchColumn();
} catch (Exception $e) {
  $showCount = $eventCount = $userCount = $trackedCount = $intCount = 0;
}
?>
        ...
        <div class="bd">
          <div class="stat-card">
            <div class="stat-row">
              ...
              <span class="stat-value"><?= number_format($showCount) ?></span>
            </div>
            <div class="stat-row">
            ...
            <div class="stat-row">
              ...
              <span class="stat-value"><?= number_format($userCount) ?></span>
            </div>
            <div class="stat-row">
              <span class="stat-label">Tracked Shows</span>
              <span class="stat-value"><?= number_format($trackedCount) ?></span>
            </div>
            <div class="stat-row">
              <span class="stat-label">Integrations</span>
              <span class="stat-value"><?= number_format($intCount) ?></span>
            </div>
          </div>
...
async function checkIntegrity() {
  ...
  try {
    ...
    results.innerHTML = '<p class="small" style="color:#f00;">✗ Error: ' + err.message + '</p>';
  }
}
...
fetch('/api/admin_activity.php')
  ...
  .catch(err => {
    console.error('User activity load failed:', err);
    document.getElementById('user-activity').innerHTML = '<p class="small" style="color:#f00;">Error loading</p>';
  });
...
(async function() {
  ...
  try {
    ...
    if (!data.ok) {
      wrap.innerHTML = '<p class="small" style="color:#f00;">Failed to load: ' + (data.error || 'unknown') + '</p>';
      return;
    }
    ...
    console.error('Module policy load failed:', err);
    wrap.innerHTML = '<p class="small" style="color:#f00;">Error: ' + err.message + '</p>';
  }
})();

console.log('Admin dashboard JS complete');
</script>
